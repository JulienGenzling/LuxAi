...
from base import Global, ActionType, is_team_sector, warp_point
...
def harvest(self):
    ...
    def set_task(ship, target_node):
        ...
        if not is_team_sector(self.team_id, target_node.x, target_node.y):
            if not has_sufficient_energy(self, ship, target_node):
...
def has_sufficient_energy(self, ship, target):
